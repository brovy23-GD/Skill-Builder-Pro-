...
public partial class AthleteDashboardPage:ContentPage{public AthleteDashboardPage(DashboardViewModel vm){InitializeComponent();BindingContext=vm;}void DashboardStageSizeChanged(object? s,EventArgs e){var phone=ResponsiveLayout.IsPhone(DashboardStage.Width);DashboardContent.Padding=ResponsiveLayout.PhonePagePadding(DashboardStage.Width);DashboardHeaderGrid.ColumnDefinitions.Clear();DashboardHeaderGrid.RowDefinitions.Clear();if(phone){DashboardHeaderGrid.ColumnDefinitions.Add(new ColumnDefinition(GridLength.Star));DashboardHeaderGrid.RowDefinitions.Add(new RowDefinition(GridLength.Auto));DashboardHeaderGrid.RowDefinitions.Add(new RowDefinition(GridLength.Auto));DashboardHeaderGrid.SetColumn(DashboardHeaderActions,0);DashboardHeaderGrid.SetRow(DashboardHeaderActions,1);DashboardHeaderActions.HorizontalOptions=LayoutOptions.Start;}else{DashboardHeaderGrid.ColumnDefinitions.Add(new ColumnDefinition(GridLength.Star));DashboardHeaderGrid.ColumnDefinitions.Add(new ColumnDefinition(GridLength.Auto));DashboardHeaderGrid.RowDefinitions.Add(new RowDefinition(GridLength.Auto));DashboardHeaderGrid.SetColumn(DashboardHeaderActions,1);DashboardHeaderGrid.SetRow(DashboardHeaderActions,0);DashboardHeaderActions.HorizontalOptions=LayoutOptions.End;}((DashboardViewModel)BindingContext).UpdateViewport(DashboardStage.Width,DashboardStage.Height);}protected override void OnAppearing(){base.OnAppearing();((DashboardViewModel)BindingContext).LoadCommand.Execute(null);}async void BellClicked(object? s,EventArgs e)=>await Shell.Current.GoToAsync(nameof(NotificationsPage));async void QuickActionClicked(object? s,EventArgs e){if(s is Button{CommandParameter:string route})await Shell.Current.GoToAsync($"//{route}");}}
...
public partial class TrophyRoomPage:ContentPage
{
 ...
 void TrophyStageSizeChanged(object? s,EventArgs e){if(TrophyStage.Width<=0)return;var phone=TrophyStage.Width<700;var rank=TrophySummaryGrid.Children[0];var spacer=TrophySummaryGrid.Children[1];var achievements=TrophySummaryGrid.Children[2];TrophySummaryGrid.ColumnDefinitions.Clear();TrophySummaryGrid.RowDefinitions.Clear();MilestoneGrid.ColumnDefinitions.Clear();MilestoneGrid.RowDefinitions.Clear();var nextMilestone=MilestoneGrid.Children[1];if(phone){TrophySummaryGrid.ColumnDefinitions.Add(new ColumnDefinition(GridLength.Star));TrophySummaryGrid.RowDefinitions.Add(new RowDefinition(GridLength.Auto));TrophySummaryGrid.RowDefinitions.Add(new RowDefinition(new GridLength(180)));TrophySummaryGrid.RowDefinitions.Add(new RowDefinition(GridLength.Auto));TrophySummaryGrid.SetColumn(rank,0);TrophySummaryGrid.SetRow(rank,0);TrophySummaryGrid.SetColumn(spacer,0);TrophySummaryGrid.SetRow(spacer,1);TrophySummaryGrid.SetColumn(achievements,0);TrophySummaryGrid.SetRow(achievements,2);MilestoneGrid.ColumnDefinitions.Add(new ColumnDefinition(GridLength.Star));MilestoneGrid.RowDefinitions.Add(new RowDefinition(GridLength.Auto));MilestoneGrid.RowDefinitions.Add(new RowDefinition(GridLength.Auto));MilestoneGrid.SetColumn(nextMilestone,0);MilestoneGrid.SetRow(nextMilestone,1);}else{TrophySummaryGrid.ColumnDefinitions.Add(new ColumnDefinition(new GridLength(.9,GridUnitType.Star)));TrophySummaryGrid.ColumnDefinitions.Add(new ColumnDefinition(new GridLength(1.2,GridUnitType.Star)));TrophySummaryGrid.ColumnDefinitions.Add(new ColumnDefinition(new GridLength(.9,GridUnitType.Star)));TrophySummaryGrid.RowDefinitions.Add(new RowDefinition(GridLength.Auto));TrophySummaryGrid.SetColumn(rank,0);TrophySummaryGrid.SetRow(rank,0);TrophySummaryGrid.SetColumn(spacer,1);TrophySummaryGrid.SetRow(spacer,0);TrophySummaryGrid.SetColumn(achievements,2);TrophySummaryGrid.SetRow(achievements,0);MilestoneGrid.ColumnDefinitions.Add(new ColumnDefinition(GridLength.Star));MilestoneGrid.ColumnDefinitions.Add(new ColumnDefinition(GridLength.Star));MilestoneGrid.RowDefinitions.Add(new RowDefinition(GridLength.Auto));MilestoneGrid.SetColumn(nextMilestone,1);MilestoneGrid.SetRow(nextMilestone,0);}}
...
public partial class TrainingPage:ContentPage,IQueryAttributable
{
 ...
 void TrainingStageSizeChanged(object? s,EventArgs e){TrainingContent.Padding=ResponsiveLayout.PhonePagePadding(TrainingStage.Width);((TrainingViewModel)BindingContext).UpdateViewport(TrainingStage.Width,TrainingStage.Height);}
 ...
 async void TrainingBuilderClicked(object? s,EventArgs e){var sport=((TrainingViewModel)BindingContext).SelectedSport??string.Empty;await Shell.Current.GoToAsync($"{nameof(TrainingBuilderPage)}?sport={Uri.EscapeDataString(sport)}");}
 async void DrillLibraryClicked(object? s,EventArgs e)=>await Shell.Current.GoToAsync(nameof(DrillLibraryPage));
...
public partial class ProfilePage:ContentPage
{
 ...
 void ProfileSizeChanged(object? sender,EventArgs e)
 {
  ...
  ProfileContent.Padding=phone?new Thickness(16,12,16,92):new Thickness(20);
