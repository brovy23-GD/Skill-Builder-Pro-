...
[ApiController, Authorize(Roles = ApplicationRoles.Coach), Route("api/coach")]
public sealed class CoachController : ControllerBase
{
    ...
    [HttpGet("teams")]
    public async Task<ActionResult<IReadOnlyCollection<CoachTeamResponse>>> GetTeams(CancellationToken cancellationToken)
    {
        if (_currentUser.UserId is not int userId) return Unauthorized();
        return Ok(await _discovery.GetCoachTeamsAsync(userId, cancellationToken));
    }

    [HttpGet("teams/{teamId:int}/roster")]
    public async Task<ActionResult<IReadOnlyCollection<AthleteSummaryResponse>>> GetRoster(int teamId, CancellationToken cancellationToken)
    {
        if (_currentUser.UserId is not int userId) return Unauthorized();
        var roster = await _discovery.GetCoachTeamRosterAsync(userId, teamId, cancellationToken);
        return roster is null ? NotFound() : Ok(roster);
    }

    [HttpPost("teams/{teamId:int}/assignments")]
    public async Task<ActionResult<DrillAssignmentResponse>> CreateTeamAssignment(
        int teamId,
        TeamAssignmentRequest request,
        CancellationToken cancellationToken)
    {
        if (_currentUser.UserId is not int userId) return Unauthorized();
        var result = await _assignmentService.CreateForTeamAsync(
            userId,
            teamId,
            new AssignmentCreateCommand(request.DrillId, null, request.ScheduledForUtc, request.DueAtUtc, request.Instructions, request.CountsTowardProgression),
            cancellationToken);
        return AssignmentControllerResult.Created(this, result);
    }

    [HttpPost("teams/{teamId:int}/assignments/selected")]
    public async Task<ActionResult<DrillAssignmentResponse>> CreateSelectedAssignment(
        int teamId,
        SelectedTeamAssignmentRequest request,
        CancellationToken cancellationToken)
    {
        if (_currentUser.UserId is not int userId) return Unauthorized();
        var result = await _assignmentService.CreateForSelectedTeamAthletesAsync(
            userId,
            teamId,
    ...
    {
        if (_currentUser.UserId is not int userId) return Unauthorized();
        if (!await _relationshipAccess.CanCoachAccessAthleteAsync(userId, athleteUserId, cancellationToken)) return NotFound();
